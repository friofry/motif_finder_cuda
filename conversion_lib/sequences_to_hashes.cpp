...
std::vector<std::string> hashes_to_sequences(const SequenceHashes &seq_hashes, bool complementary)
{
    ...
    for (int i = 0; i < seq_hashes.count; i++) {
        string seq;
        uint32_t start = seq_hashes.seq_begins[i];
        uint32_t l = complementary ? seq_hashes.lengths[i] / 2 : seq_hashes.lengths[i];
        ...
        if (complementary) {
            for (uint32_t j = 0; j < l; j++) {
                auto s = compl_hash_to_string(seq_hashes.hashes[start + j + l]);
                if (j < l - 1) {
                    seq += s[0];
                } else {
                    seq += s;
