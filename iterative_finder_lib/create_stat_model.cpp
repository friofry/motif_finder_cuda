                               ...
                               double correction)
{
    if (!use_real_nucl_frequences) {
        return make_shared<IdealStatModel>(sequences, complementary, use_binom_instead_of_chi2, correction);
    } else {
        if (markov_level == 0) {
            return make_shared<BernulliStatModel>(sequences, complementary, use_binom_instead_of_chi2, correction);
        } else if (markov_level < 5) {
            return make_shared<MarkovStatModel>(sequences, complementary, markov_level, use_binom_instead_of_chi2, correction);
        }
