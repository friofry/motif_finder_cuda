...
StatModelPtr create_stat_model(const std::string &model, const std::vector<std::string> &sequences, bool complementary)
{
    if (model == "bernulli") {
        ...
        return make_shared<MarkovStatModel>(sequences, complementary, 1);
    } else if (model == "markov2") {
        return make_shared<MarkovStatModel>(sequences, complementary, 2);
    } else if (model == "markov3") {
        return make_shared<MarkovStatModel>(sequences, complementary, 3);
    } else {
        // "ideal"
        return std::make_shared<IdealStatModel>(sequences, complementary);
    }
}